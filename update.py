...
def is_connected(host="8.8.8.8", port=53, timeout=3):
    ...
    Parameters:
      host (str): The IP address of the host to connect to. Default is Google's public DNS, 8.8.8.8.
      port (int): The network port to use. Default is 53 (DNS service).
    ...
    """
    try:
        # Set default timeout and attempt connection
        socket.setdefaulttimeout(timeout)
        # Create a socket object and connect
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect((host, port))
        sock.close()  # Cleanly close the connection
        return True
    except OSError:
        return False
